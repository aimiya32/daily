...
모델 배정:
너(현재 세션)의 모델이 Fable인 경우 → Worker: Opus
너(현재 세션)의 모델이 Opus인 경우 → Worker: Sonnet
너(현재 세션)의 모델이 Sonnet인 경우 → 위임하지 말고 직접 처리한다
위임 오버헤드가 더 큰 작업(한두 줄 수정 등)은 Worker에게 넘기지 말고 현재 세션에 설정된 모델로 직접 처리한다
...
Agent 도구로 위임하고 model은 위 배정 기준에 따라 지정한다(너의 모델이 Fable이면 opus, Opus면 sonnet)
